...
import numpy as np
import models.VAE_functions as VAE_functions


def HVAE_graph(data, model_name, types_file, batch_size, learning_rate=1e-3, z_dim=2, y_dim=1, s_dim=2, y_dim_partition=[]):
    
    #We select the model for the VAE
    print('[*] Importing model: ' + model_name)
    model = __import__(f'{model_name}') # model_HIVAE_inputDropout
    
    #Load placeholders
    print('[*] Defining placeholders')
    batch_data_list, batch_data_list_observed, miss_list, tau, tau2, types_list = VAE_functions.place_holder_types(data, batch_size)
