...
class Model(torch.nn.Module):
    ...
    def __init__(self, input_dim, model_params_dict):
        ...
        # q(xmis|x, y) 
        self.fc_xobsy_h = torch.nn.Linear(self.r_cat_dim, self.h_dim)
        self.fc_hxobsy_h = torch.nn.Linear(self.h_dim, self.h_dim)
        self.fc_h_xmis = torch.nn.Linear(self.h_dim, self.input_dim*2)
        
        # p(xmis|y) 
        self.fc_hxmis_y = torch.nn.Linear(self.r_cat_dim, self.h_dim)
        self.fc_hxmis_h = torch.nn.Linear(self.h_dim, self.h_dim)
        self.fc_xmis_h = torch.nn.Linear(self.h_dim, self.input_dim*2)
        ...
        self.fc_h_xm = torch.nn.Linear(self.h_dim, self.input_dim_wm + self.input_dim)
    ...
    def pxmis_graph(self, y, L=1):
        # p(x|y)
        h_xmis = F.relu(self.fc_hxmis_y(y))
        h_xmis = F.relu(self.fc_hxmis_h(h_xmis))
        xmis_prior = F.relu(self.fc_xmis_h(h_xmis))
        xmis_mu_prior, xmis_logvar_prior = torch.split(xmis_prior, self.input_dim, dim=-1) 
        
        return xmis_mu_prior, torch.clamp(xmis_logvar_prior, -15, 15) #prevent numerical inaccuracies
    ...
    def forward(self, x, m, test_mode=False, L=1):
        ...
        for i in range(self.r_cat_dim):
            ...
            if not test_mode:
                ...
                xmis_mu_prior[i], xmis_logvar_prior[i] = self.pxmis_graph(y, L)
                zm_prior[i], zv_prior[i], recon[i] = self.decoder(z[i], y, test_mode, L)
                xmis_mu[i] = recon[i]['xmis']
                xmis_logvar[i] = torch.log(torch.tensor([0.25]))
                eps = torch.randn_like(xmis_mu[i]).to(xm.device)
                xmis[i] = xmis_mu[i] + eps * torch.sqrt(torch.exp(torch.clamp(xmis_logvar[i], -15, 15))) 
            else:
                z[i], zm[i], zv[i] = self.qz_graph(xm, y, test_mode, L)
        ...
        variational_params = {
            ...
            'xmis_mu_prior': torch.stack(xmis_mu_prior), 
            'xmis_logvar_prior': torch.stack(xmis_logvar_prior), 
            'z_mu': torch.stack(zm),
